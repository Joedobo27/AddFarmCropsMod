...
class BytecodeTools extends Bytecode {
    ...
    int addFloatIndex(float floatValue){
        int indexReference = this.getConstPool().addFloatInfo(floatValue);
        this.add((indexReference >>> 8) & 0xFF, indexReference & 0xFF);
        return indexReference;
    }

    int addFloat(float floatValue) {
        final float EPSILON = 0.00000001f;
        int index = -1;
        if (Math.abs(0 - floatValue) < EPSILON)
            this.addOpcode(Opcode.FCONST_0);
        else if (Math.abs(1 - floatValue) < EPSILON)
            this.addOpcode(Opcode.FCONST_1);
        else if (Math.abs(2 - floatValue) < EPSILON)
            this.addOpcode(Opcode.FCONST_2);
        else {
            this.addOpcode(Opcode.LDC_W);
            index = this.addFloatIndex(floatValue);
        }
        return index;
    }
    ...
    int findIntegerIndex(int opcode, int integerValue){
        int indexReference = IntStream.range(1, this.getConstPool().getSize())
                .filter(value -> this.getConstPool().getTag(value) == ConstPool.CONST_Integer)
                .filter(value -> this.getConstPool().getIntegerInfo(value) == integerValue)
                .findFirst()
                .orElseThrow(NoMatchingConstPoolIndexException::new);
        this.addOpcode(opcode);
        this.add((indexReference >>> 8) & 0xFF, indexReference & 0xFF);
